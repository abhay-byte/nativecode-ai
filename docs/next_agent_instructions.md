...
- First-time setup fails at **Step D (Initializing Host Environment)** with:
  `proot error: execve("/data/data/com.ivarna.nativecode/files/usr/bin/bash"): Permission denied`

## Root Cause of Step D Failure
1. `proot` is a host-side binary launched via the linker (`/system/bin/linker64 /data/data/.../proot`).
2. When `proot` attempts to execute the guest binary (`/data/data/com.ivarna.nativecode/files/usr/bin/bash`), it forks and calls `execvp` on the host to execute the guest binary (or the unbundled loader at `/data/data/com.ivarna.nativecode/files/usr/libexec/proot/loader`).
3. Since both `bash` and `loader` reside in `/data/data/com.ivarna.nativecode/files/` (a writable app directory), Android blocks direct execution of these binaries with `Permission denied` under Target SDK 36.
...
1. **Package Binaries in APK `jniLibs` (Recommended / Standard W^X Bypass)**
   ...
   - When the APK is built and installed, Android will extract these to the read-only `/data/data/com.ivarna.nativecode/lib/` directory.
