...
How to reach both Linux environments installed by **NativeCode** (`com.zenithblue.nativecode`) on a connected device.
...
| Item | Path |
|------|------|
| App package | `com.zenithblue.nativecode` |
| Host prefix | `/data/data/com.zenithblue.nativecode/files/usr` |
...
App prefs (`/data/data/com.zenithblue.nativecode/shared_prefs/nativecode_prefs.xml`):
...
grep -E 'linux_method|proot_dir|chroot_dir|chroot_installed' \
  /data/data/com.zenithblue.nativecode/shared_prefs/nativecode_prefs.xml
...
Package is **`com.zenithblue.nativecode`**. Stock Termux is **`com.termux`** and must **not** appear in host paths.
...
| `TERMUX__PREFIX` | `/data/data/com.zenithblue.nativecode/files/usr` |
| `TERMUX__HOME` | `/data/data/com.zenithblue.nativecode/files/home` |
...
```bash
adb -s "$SERIAL" shell 'stat -c "%u %U" /data/data/com.zenithblue.nativecode'
# e.g. 10510 u0_a510
```
...
```bash
adb -s "$SERIAL" shell 'find /data/app -path "*com.zenithblue.nativecode*" -name libproot.so'
```
...
```bash
LIBDIR=$(adb -s "$SERIAL" shell 'find /data/app -path "*com.zenithblue.nativecode*" -name libproot.so' | tr -d '\r' | xargs dirname)
