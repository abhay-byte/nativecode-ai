...
object ChrootCommandBuilder {
    ...
    fun build(
        ...
        val rootInner: String = when {
            isInteractive && u == "root" ->
                "exec sh $CHROOT_HELPER login --user root --shell bash"
            isInteractive ->
                "exec sh $CHROOT_HELPER login --user flux --shell zsh"
     ...
     * Reject `$` / backticks / double-quotes / newlines / backslash — those need b64.
     */
